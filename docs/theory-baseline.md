...
| `src/auto.ts` | N=3、k=1、K=2、终局答案触发 PPT | N=3、k=1、K=2 来自作者 self-verification/Bo3 配置；论文主实验常用配置为 G=20、K=8、C=3。代码注释明确标记为 Bo3 compatibility defaults。 |
| `_ref/` | 作者仓库本地参考副本 | 最新核验固定在提交 `8db8a114355a9d7fdf9a8d1d5c87f6aeebd18770`；上一轮兼容快照为 `115de305f23ed89bc42e86e010853c40059f3f7d`。 |
| `docs/` | 检查时仅有空的 `docs/research/` 目录 | 仓库缺少已跟踪的文档落盘约定，因此采用任务指定的 `docs/theory-baseline.md`。 |

本次代码映射基于共享工作区当前内容；本文只新增本文件。
...
- 论文的主要验证实验针对完整 agent trajectory。插件在共享工具历史之后只扩展终局回答候选，属于产品化候选分布；任何论文指标迁移声明都需要本插件场景的独立评测支持。[论文 §3.1–§3.2，pp. 4–7](https://arxiv.org/pdf/2607.05391#page=4)
...
2. `argmax` token、模型正文中打印的单个字母、离散多数票均会改变 Eq. (3.1) 的估计量。
...
- N=3、k=1、K=2 是 self-verification 的成本/质量配置，不构成全部论文实验的统一默认值。
- 独立 verifier model 是支持项；同模型 self-verification 也是支持项。遥测需记录实际 generator/verifier 身份。
- 候选采样需要相互独立，并共享相同任务与前置证据条件。会话链式状态、随机种子复用和缓存复用需要避免把候选收缩成重复样本。
- 同一 PPT 中的候选需要处于相同轨迹边界。终态答案锦标赛只接纳终态候选；继续提出工具动作的替代采样单独计数。
- 本插件只在终局答案触发候选扩展与 PPT，工具调用回合直接继续 agent loop。该策略属于插件成本控制边界，质量结论依赖本插件评测。
...
| INV-10 | `N+k(N-k)+C(k,2)` 是比较数上界；精确计数扣除 ring 与原始 pivot edge 集的有向交集。N=3、k=1 固定为 4。 | 多组 N/k/ring 验证集合差与上界；Bo3 固定断言 4；另报 unique keys/provider calls。 | [论文 §3.2，p. 7](https://arxiv.org/pdf/2607.05391#page=7)；[Appendix B.2，Algorithm 1，p. 27](https://arxiv.org/pdf/2607.05391#page=27) |
...
| 将 N=3、k=1、K=2 写成通用论文默认值 | 该组合来自作者 Bo3 self-verification；论文主实验使用更高 K，并按场景选择 k。文档与遥测写明具体配置来源。 | [论文 §4，pp. 7–10](https://arxiv.org/pdf/2607.05391#page=7)；[作者 Bo3 脚本 L1–35](https://github.com/llm-as-a-verifier/llm-as-a-verifier/blob/115de305f23ed89bc42e86e010853c40059f3f7d/scripts/run_bo3.py#L1-L35) |
...
| 以多数票捷径跳过 PPT | 已核验的论文、作者仓库和作者官方文档未提供“文本答案多数票可替代连续期望奖励 + PPT”的理论依据。多数票应作为独立策略命名、单独遥测和独立评测；进入该路径的结果不计入 PPT 论文等价指标。 | [论文 §3.2，pp. 5–7](https://arxiv.org/pdf/2607.05391#page=5)；[作者 Best-of-N 官方文档](https://llm-as-a-verifier.com/docs/basic_usage/best_of_n_selection.html)；[作者 PPT 官方文档](https://llm-as-a-verifier.com/docs/advanced_features/pivot_tournament.html) |
| 把插件终局答案选择结果直接外推为论文 benchmark 提升 | 候选生成、轨迹长度、证据截断、模型和任务分布均影响外部效度。发布指标需使用本插件固定协议复测。 | [论文 §5 与 Appendix A，pp. 10–22、26](https://arxiv.org/pdf/2607.05391#page=10) |
...
5. 最早成功候选 fallback 与 capability failure 使用独立 decision path；若未来引入多数票，也必须采用独立 path。文本字母回退和 0.5 tie 进入 `scoreSources`，并令 `paperEquivalent=false`。论文等价指标只聚合真实 logprob 期望经过 PPT 产生的决策。
