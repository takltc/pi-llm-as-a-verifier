...
The recommended Coding Agent granularity is a **consequential action batch**: observations use one sample; all declarative edits/tool calls in one response are selected once and replayed as one winning batch. Actions that depend on a preceding tool result remain separate decisions. Defaults are `N=3, pivots=2, C=K=1, G=20`; this is an engineering PRM schedule, not a proven universally optimal stage policy. See the [granularity study](docs/granularity-selection.md) for the evidence and replay comparison.
...
Cache writes use OMP's native process lock, merging under the lock before atomic replacement. Process exit releases ownership automatically. Stop older OMP sessions before upgrading. If a legacy `.lock` directory is reported, remove it only after its old writers have stopped; the plugin refuses to reclaim it automatically. Retain native `.lock` files, especially while writers are running. Cache version 7 re-scores old keys to exclude results produced by the former invalid-probability parsing.

Verifier input preserves the full visible conversation, tool results, and candidates without silent character truncation. Provider context limits surface through explicit error or fallback handling. `usage.calls` counts HTTP attempts for that selection, including retries and responses without usage; token totals contain only provider-reported usage. Concurrent selections have separate counters.
